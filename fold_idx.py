...
for i in range(1,11):
    ...
    for each in train_idx:
        _write_msg('dataset/TESTLIU4/10fold_idx', 'train_idx-%s.txt' % str(i), str(each))
    for each in test_idx:
        _write_msg('dataset/TESTLIU4/10fold_idx', 'test_idx-%s.txt' % str(i), str(each))
